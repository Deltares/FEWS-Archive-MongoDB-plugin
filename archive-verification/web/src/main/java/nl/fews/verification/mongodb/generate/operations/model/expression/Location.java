	...
	public void execute(){
		...
		String queries = studyDocument.getList("Forecasts", String.class).stream().map(s -> {
			...
			return forecastDocument.getList("Filters", Document.class).stream().map(
				f -> String.format("        Odbc.Query(Source, \"SELECT s.location, l.* FROM %s.%s s INNER JOIN Verification.%s l ON l.locationId=s.locationId\")", Settings.get("archiveDB"), String.format("%s_%s_Location_%s", study, forecastDocument.getString("ForecastName"), f.getString("FilterName")), String.format("%s_Location", study))).collect(Collectors.joining(",\n"));
