...
public final class IsOriginalObserved implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		Mongo.insertOne("output.PowerQuerySql", new Document("Study", study).append("Name", "IsOriginalObserved").append("Month", "").append("Expression", Arrays.stream(template.replace("\r", "").split("\n")).toList()));
	}
