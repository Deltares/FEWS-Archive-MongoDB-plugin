...
public final class Measure implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		Mongo.insertOne("output.PowerQuerySql", new Document("Study", study).append("Name", "Measure").append("Month", "").append("Expression", Arrays.stream(template.replace("\r", "").split("\n")).toList()));
	}
