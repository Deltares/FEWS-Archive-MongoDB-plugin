...
public class DimensionIsOriginalObserved {
	...
	public List<Document> dimensionIsOriginalObservedN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("dimension.IsOriginalObserved", new Document(), Conversion.getProjection(e)).sort(new Document("isOriginalObserved", 1)).spliterator(), false).toList();
	}
