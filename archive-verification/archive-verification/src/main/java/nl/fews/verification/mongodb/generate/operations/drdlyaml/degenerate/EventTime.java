...
public final class EventTime implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		var union = StreamSupport.stream(Mongo.find("Forecast", new Document("Name", new Document("$in", studyDocument.getList("Forecasts", String.class)))).spliterator(), false).flatMap(s ->
			s.getList("Filters", Document.class).stream().map(f -> String.format("view.verification.%s.%s|%s|EventTime|%s", environment, study, s.getString("ForecastName"), f.getString("FilterName")))).toList();
