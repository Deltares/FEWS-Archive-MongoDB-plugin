...
public class TemplateCube {
	...
	public List<Document> templateCubeN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("template.Cube", new Document(), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).collect(Collectors.toList());
	}
