...
public class Seasonality {
	...
	public List<Document> seasonalityN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("Seasonality", new Document(), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).collect(Collectors.toList());
	}
