	...
	public void execute(){
		...
		String queries = studyDocument.getList("Forecasts", String.class).stream().map(s -> {
			...
			return forecastDocument.getList("Filters", Document.class).stream().map(
				f -> String.format("        SELECT forecastId, forecast, ensemble, ensembleMember FROM %s.`%s`", Settings.get("archiveDb"), String.format("%s_%s_Forecast_%s", study, forecastDocument.getString("ForecastName"), f.getString("FilterName")))).collect(Collectors.joining(" UNION ALL\n"));
		}).collect(Collectors.joining(" UNION ALL\n"));
		if(!studyDocument.getString("Normal").isEmpty()){
			queries = String.format("%s UNION ALL\n        SELECT 'Normal' AS forecastId, 'Normal' AS forecast, '' AS ensemble, '' AS ensembleMember", queries);
		}
