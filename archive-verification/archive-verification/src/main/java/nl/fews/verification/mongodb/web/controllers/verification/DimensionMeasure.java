...
public class DimensionMeasure {
	...
	public List<Document> dimensionMeasureN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("dimension.Measure", new Document(), Conversion.getProjection(e)).sort(new Document("measureId", 1)).spliterator(), false).collect(Collectors.toList());
	}
