...
public class OutputView {
	...
	public List<Document> outputViewNByStateEnvironmentStudy(@Argument String state, @Argument String environment, @Argument String study, DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("output.View", new Document("State", state).append("Environment", environment).append("Study", study), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).toList();
	}
	...
	public List<Document> outputViewNByStateEnvironment(@Argument String state, @Argument String environment, DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("output.View", new Document("State", state).append("Environment", environment), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).toList();
	}

	@QueryMapping
	public List<Document> outputViewN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("output.View", new Document(), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).toList();
	}
