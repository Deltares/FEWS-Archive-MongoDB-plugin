...
public class FewsLocations {
	...
	public List<Document> fewsLocationsN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("fews.Locations", new Document(), Conversion.getProjection(e)).spliterator(), false).peek(r -> r.put("lastUpdated", Conversion.dateToOffsetDateTime(r.getDate("lastUpdated")))).collect(Collectors.toList());
	}
}
