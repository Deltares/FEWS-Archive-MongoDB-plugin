...
public final class ForecastTime implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		var union = StreamSupport.stream(Mongo.find("Forecast", new Document("Name", new Document("$in", studyDocument.getList("Forecasts", String.class)))).spliterator(), false).flatMap(s ->
			s.getList("Filters", Document.class).stream().map(f -> String.format("view.verification.%s.%s|%s|ForecastTime|%s", environment, study, s.getString("ForecastName"), f.getString("FilterName")))).collect(Collectors.toList());

		var template = String.join("\n", Mongo.findOne("template.DrdlYaml", new Document("Type", "Degenerate").append("Name", "ForecastTime")).getList("Template", String.class));
		template = template.replace("{database}", Settings.get("archiveDb"));
		template = template.replace("{table}", String.format("%s_ForecastTime", study));
		...
		IO.writeString(Path.of(Settings.get("drdlYamlPath"), String.format("%s_ForecastTime.drdl.yml", study)), template);
	}
