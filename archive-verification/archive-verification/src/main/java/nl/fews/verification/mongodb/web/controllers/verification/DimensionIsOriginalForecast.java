...
public class DimensionIsOriginalForecast {
	...
	public List<Document> dimensionIsOriginalForecastN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("dimension.IsOriginalForecast", new Document(), Conversion.getProjection(e)).sort(new Document("isOriginalForecast", 1)).spliterator(), false).collect(Collectors.toList());
	}
