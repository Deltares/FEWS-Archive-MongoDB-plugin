...
public final class Location implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		if(acquisitionType.equals("mongodb"))
			IO.writeString(Path.of(Settings.get("drdlYamlPath"), String.format("%s_Location_%s.drdl.yml", study, database)), t);
		else if (acquisitionType.equals("csv"))
			Mongo.insertOne("output.DrdlYaml", new Document("Study", study).append("Name", String.format("%s_%s_%s", study, name, database)).append("Expression", Arrays.stream(t.replace("\r", "").split("\n")).toList()));
	}
