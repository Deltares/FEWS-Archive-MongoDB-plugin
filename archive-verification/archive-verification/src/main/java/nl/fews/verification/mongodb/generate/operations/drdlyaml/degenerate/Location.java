...
public final class Location implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		if(studyDocument.getString("Cube").equals("Default"))
			...
			Mongo.insertOne("output.DrdlYaml", new Document("Study", study).append("Name", String.format("%s_Location_%s", study, name)).append("Expression", Arrays.stream(t.replace("\r", "").split("\n")).toList()));
	}
