...
public final class Location implements IExecute, IPredecessor {
	...
	@Override
	public void execute(){
		String template = String.join("\n", Mongo.findOne("template.DrdlYaml", new Document("Database", Settings.get("verificationDb")).append("Type", "Dimension").append("Name", "Location")).getList("Template", String.class));
		...
		List<String> columns = attributes.stream().map(s -> String.format("""
      - Name: %s
        MongoType: %s
        SqlName: %s
        SqlType: %s
""", s, Conversion.getBsonType(locationAttributeTypes.get(s, "String")), s, Conversion.getSqlType(Conversion.getBsonType(locationAttributeTypes.get(s, "String"))))).toList();
		...
		IO.writeString(Path.of(Settings.get("drdlYamlPath"), String.format("%s_Location.drdl.yml", study)), template);
	}
