...
public final class ForecastTime implements IExecute, IPredecessor {
	...
	public void execute(){
		...
		Mongo.insertOne("output.PowerQuerySql", new Document("Study", study).append("Name", "ForecastTime").append("Month", "").append("Expression", Arrays.stream(template.replace("\r", "").split("\n")).collect(Collectors.toList())));
	}
