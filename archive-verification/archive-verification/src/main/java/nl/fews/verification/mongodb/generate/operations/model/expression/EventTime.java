	...
	public void execute(){
		...
		String queries = studyDocument.getList("Forecasts", String.class).stream().map(s -> {
			...
			return forecastDocument.getList("Filters", Document.class).stream().map(
				f -> String.format("        SELECT * FROM %s.`%s`", Settings.get("archiveDb"), String.format("%s_%s_EventTime_%s", study, forecastDocument.getString("ForecastName"), f.getString("FilterName")))).collect(Collectors.joining(" UNION ALL\n"));
