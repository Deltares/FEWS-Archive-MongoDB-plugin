...
public class OutputSql {
	...
	public List<Document> outputSqlNByStudy(@Argument String study, DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("output.Sql", new Document("Study", study), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).collect(Collectors.toList());
	}

	@QueryMapping
	public List<Document> outputSqlNByStudyName(@Argument String study, @Argument String name, DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("output.Sql", new Document("Study", study).append("Name", name), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).collect(Collectors.toList());
	}

	@QueryMapping
	public List<Document> outputSqlN(DataFetchingEnvironment e){
		return StreamSupport.stream(Mongo.find("output.Sql", new Document(), Conversion.getProjection(e)).sort(new Document("Name", 1)).spliterator(), false).collect(Collectors.toList());
	}
