...
const { result, loading, error, refetch } = useQuery(gql`query outputPowerQuerySqlN {outputPowerQuerySqlN {_id, Study, Name, Month, Expression}}`)
const selected = ref({})
const success = ref(null)
const sorted = computed(() => result?.value?.outputPowerQuerySqlN ? result.value.outputPowerQuerySqlN.slice().sort((a, b) => `${a.Study}_${a.Name}_${a.Month}`.localeCompare(`${b.Study}_${b.Name}_${b.Month}`)) : [])

const createMutation = useMutation(gql`mutation createOutputPowerQuerySql($study: String!, $name: String!, $month: String!, $expression: String!) {createOutputPowerQuerySql(study: $study, name: $name, month: $month, expression: $expression)}`)
const updateMutation = useMutation(gql`mutation updateOutputPowerQuerySql($_id: ID!, $study: String!, $name: String!, $month: String!, $expression: String!) {updateOutputPowerQuerySql(_id: $_id, study: $study, name: $name, month: $month, expression: $expression)}`)
const deleteMutation = useMutation(gql`mutation deleteOutputPowerQuerySql($_id: ID!) {deleteOutputPowerQuerySql(_id: $_id)}`)
...
async function create() {
  ...
  selected.value._id = result?.data ? result.data.createOutputPowerQuerySql : selected.value._id
}
...
<div class="pa-4 pt-2">
  <div class="bg-blue-darken-2 rounded-lg text-center pa-2"><h3>OutputPowerQuerySql Editor</h3></div>
