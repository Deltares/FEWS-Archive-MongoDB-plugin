...
const LIST = `query {observedN {_id, Name, Collection, Filters}}`
const TEST = `query ($collection: String!, $filters: JSON!) {observedTest(collection: $collection, filters: $filters){FilterName, Success}}`
const CREATE = `mutation ($name: String!, $collection: String!, $filters: JSON!) {createObserved(name: $name, collection: $collection, filters: $filters)}`
const UPDATE = `mutation ($_id: ID!, $name: String!, $collection: String!, $filters: JSON!) {updateObserved(_id: $_id, name: $name, collection: $collection, filters: $filters)}`
const DELETE = `mutation ($_id: ID!) {deleteObserved(_id: $_id)}`
...
async function run(mutation) {
  ...
  try {
    ...
    items.value = (await graphql(LIST)).observedN
  }
...
const create = () => run(async () => {
  ...
  selected.value._id = data.createObserved
  return data
})
...
async function test() {
  ...
  try {
    ...
    const results = (await graphql(TEST, {collection: Collection, filters: JSON.parse(Filters)})).observedTest
...
<div class="pa-4 pt-2">
  <div class="bg-blue-darken-2 rounded-lg text-center pa-1"><h3 class="ma-1">Observed Editor</h3></div>
