...
async function update() {
  const {_id, Name, Observed, Forecasts, Seasonalities, Class, LocationAttributes, ForecastStartMonth, ForecastEndMonth, Time, Value, Normal, Cube, Active, ReprocessDays} = selected.value
...
<div class="pa-4 pt-2">
  ...
  <div class="input">
    ...
    <div v-if="forecast.result.value" class="d-flex w-100 mt-2"><label for="i-forecasts" class="border rounded-lg pa-2 input-label">Forecasts</label><select id="i-forecasts" multiple class="border rounded-lg pa-2 flex-grow-1 ml-2 input-data" v-model="selected.Forecasts"><option v-for="x in forecast.result.value.forecastN" :key="x.Name" :value="x.Name" :label="x.Name"/></select></div>
