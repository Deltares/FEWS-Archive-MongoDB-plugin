...
async function create() {
  ...
  const result = await mutate(() => createMutation.mutate({ name: Name, attributes: JSON.parse(Attributes) }))
...
async function update() {
  ...
  await mutate(() => updateMutation.mutate({ _id: _id, name: Name, attributes: JSON.parse(Attributes) }))
}
