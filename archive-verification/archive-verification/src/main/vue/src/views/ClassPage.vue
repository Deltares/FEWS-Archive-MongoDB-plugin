...
const sorted = computed(() => result?.value?.classN ? result.value.classN.slice().sort((a, b) => a.Name.localeCompare(b.Name)) : [])
...
async function create() {
  ...
  const result = await mutate(() => createMutation.mutate({ name: Name, locations: JSON.parse(Locations) }))
  selected.value._id = result?.data ? result.data.createClass : selected.value._id
}

async function update() {
  const {_id, Name, Locations} = selected.value
  await mutate(() => updateMutation.mutate({ _id: _id, name: Name, locations: JSON.parse(Locations) }))
}
