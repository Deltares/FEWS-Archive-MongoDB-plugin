...
docker run --rm -v "$PWD:/io" quay.io/pypa/manylinux2014_x86_64 bash -lc "
    set -e
    cd /opt/python/cp313-cp313/bin
    python -m pip install --upgrade pip wheel build auditwheel
    pip wheel -r /io/requirements.txt -w /io/wheelhouse
    pip wheel /io/models/GraphCastOperationalIfs/graphcast.zip -w /io/wheelhouse
    pip wheel /io/. -w /io/wheelhouse --no-build-isolation
...
# CLEANUP
rm -rf tva_graphcast.egg-info build/bdist.linux-x86_64 build/lib
