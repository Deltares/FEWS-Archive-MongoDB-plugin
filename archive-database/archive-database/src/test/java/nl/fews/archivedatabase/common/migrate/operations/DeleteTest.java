...
//	}
	...
	void deleteMetaDatas() throws ExecutionException, InterruptedException {
		...
		Delete.deleteMetaDatas(new LinkedHashMap<>(), MetaDataUtil.getExistingMetaDataFilesDb());
...
//		Insert.insertMetaData(entry.getKey(), entry.getValue());
//		assertEquals(1, MetaDataUtil.getExistingMetaDataFilesDb().size());
//		Database.updateMany(Collection.MigrateMetaData.toString(), new Document(), new Document("$set", new Document("committed", false)));
