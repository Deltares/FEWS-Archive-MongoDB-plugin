...
class ScalarExternalForecastingTest {
	...
	@Test
	void getRoot() {
		Map.Entry<File, Date> entry = MetaDataUtil.getExistingMetaDataFilesFs().entrySet().stream().filter(s -> s.getKey().toString().contains("external_forecasts") && s.getKey().toString().contains("scalar")).findFirst().orElse(null);
		...
		Assertions.assertNotNull(new ScalarExternalForecasting().getRoot(document, timeSeriesSet, new ArrayList<>(), new Document()));
	}
	...
	void getMetaData() {
		...
		Assertions.assertNotNull(new ScalarExternalForecasting().getMetaData(document, timeSeriesSet, netcdfFile.getValue().getValue1()));
	}
	...
	void getEvents() {
		...
		Document document = NetcdfUtil.getTimeSeriesDocuments(netcdfFile.getKey()).stream().findFirst().get();
		JSONObject timeSeriesSet = TimeSeriesSetsUtil.getDecomposedTimeSeriesSets(netcdfFile.getKey()).getJSONObject(0);
		Assertions.assertNotNull(new ScalarExternalForecasting().getEvents(document.getList("timeseries", Document.class), timeSeriesSet));
	}
	...
	void getRunInfo() {
		...
		Assertions.assertNotNull(new ScalarExternalForecasting().getRunInfo(runInfo));
	}
}