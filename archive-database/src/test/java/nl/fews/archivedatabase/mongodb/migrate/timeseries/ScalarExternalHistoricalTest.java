...
class ScalarExternalHistoricalTest {
	...
	void getRoot() {
		...
		Assertions.assertNotNull(new ScalarExternalHistorical().getRoot(document, timeSeriesSet, new ArrayList<>(), new Document()));
	}
	...
	void getMetaData() {
		...
		Assertions.assertNotNull(new ScalarExternalHistorical().getMetaData(document, timeSeriesSet, netcdfFile.getValue().getValue1()));
	}
	...
	void getEvents() {
		...
		Document document = NetcdfUtil.getTimeSeriesDocuments(netcdfFile.getKey()).stream().findFirst().get();
		JSONObject timeSeriesSet = TimeSeriesSetsUtil.getDecomposedTimeSeriesSets(netcdfFile.getKey()).getJSONObject(0);
		Assertions.assertNotNull(new ScalarExternalHistorical().getEvents(document.getList("timeseries", Document.class), timeSeriesSet));
	}
	...
	void getRunInfo() {
		...
		Assertions.assertNotNull(new ScalarExternalHistorical().getRunInfo(runInfo));
	}
}