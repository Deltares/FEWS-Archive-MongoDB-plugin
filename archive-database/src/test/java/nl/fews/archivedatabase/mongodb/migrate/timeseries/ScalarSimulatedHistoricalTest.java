...
class ScalarSimulatedHistoricalTest {
	...
	void getRoot() {
		...
		Assertions.assertNotNull(new ScalarSimulatedHistorical().getRoot(document, timeSeriesSet, new ArrayList<>(), new ScalarSimulatedHistorical().getRunInfo(runInfo)));
	}
	...
	void getMetaData() {
		...
		Assertions.assertNotNull(new ScalarSimulatedHistorical().getMetaData(document, timeSeriesSet, netcdfFile.getValue().getValue1()));
	}
	...
	void getEvents() {
		...
		Document document = NetcdfUtil.getTimeSeriesDocuments(netcdfFile.getKey()).stream().findFirst().get();
		JSONObject timeSeriesSet = TimeSeriesSetsUtil.getDecomposedTimeSeriesSets(netcdfFile.getKey()).getJSONObject(0);
		Assertions.assertNotNull(new ScalarSimulatedHistorical().getEvents(document.getList("timeseries", Document.class), timeSeriesSet));
	}
	...
	void getRunInfo() {
		...
		JSONObject metaData = MetaDataUtil.readMetaData(entry.getKey());
		JSONObject runInfo = RunInfoUtil.readRunInfo(RunInfoUtil.getRunInfoFile(metaData));
		Assertions.assertNotNull(new ScalarSimulatedHistorical().getRunInfo(runInfo));
	}
}