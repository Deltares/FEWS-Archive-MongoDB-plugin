...
public class MongoDbArchiveDatabasePlugin implements ArchiveDatabase {
	 ...
	 */
	@Override
	public void executeActivity(String areaId, String sourceId){
		//UNUSED IN EXPORT
	}
	...
	private void insertTimeSeries(TimeSeriesArrays<TimeSeriesHeader> timeSeriesArrays, TimeSeriesType timeSeriesType, String areaId, String sourceId) {
		try{
			...
			Synchronize synchronize = (Synchronize)Class.forName(String.format("%s.%s.%s", BASE_NAMESPACE, "operations", TimeSeriesTypeUtil.getTimeSeriesTypeSyncType(timeSeriesType))).getConstructor().newInstance();
